...
def parse_args():
    ...
    p.add_argument("--prior-database-dir", type=Path, nargs='+', default=None,
                   help="One or more flat prior database directories -- pass several to combine "
                        ...
                        "[[nre-training-imbalance]] memory), e.g. one nre_database_prior_capped_* "
                        "per coeval-box seed. Each is treated as its own set of catalogs for "
                        "--weight-by-catalog-count purposes. Required when --prior-only is set.")
...
def main():
    ...
    # list when set -- lets multiple independent-box prior databases combine
    # into one training set, each still reweighted independently per-catalog)
    if args.prior_only:
